...
def file_exists(fname):
    return is_valid_image_file(fname) and os.path.exists(os.path.join(IMAGE_DIR, fname))
...
# --- TensorFlow image loading pipeline ---
def decode_image(filename, label):
    def _decode(filename_str):
        try:
            image_path = tf.strings.join([IMAGE_DIR, filename_str])
            ...
            print(f"❌ Error decoding image: {filename_str.numpy().decode()} — {e}")
            raise e

    image = tf.py_function(_decode, [filename], tf.float32)
    image.set_shape((*IMAGE_SIZE, 3))  # Ensure shape is known
    return image, label
